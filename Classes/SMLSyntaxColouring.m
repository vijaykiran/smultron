...
{	
	...
	NSFetchRequest *request = [[entityDescription managedObjectModel] fetchRequestFromTemplateWithName:@"syntaxDefinitionName" substitutionVariables:[NSDictionary dictionaryWithObject:[SMLDefaults valueForKey:@"SyntaxColouringPopUpString"] forKey:@"NAME"]];
...
- (void)textDidChange:(NSNotification *)notification
{
	...
	if ([completeString length] < 2) {
		[SMLInterface updateStatusBar]; // One needs to call this from here as well because otherwise it won't update the status bar if one writes one character and deletes it in an empty document, because the textViewDidChangeSelection delegate method won't be called.
	}
...
- (void)textViewDidChangeSelection:(NSNotification *)aNotification
{
	...
	[SMLCurrentProject setLastTextViewInFocus:textView];
...
- (void)checkIfCanUndo
{
	if (![undoManager canUndo]) {
		[SMLCurrentDocument setValue:[NSNumber numberWithBool:NO] forKey:@"isEdited"];
...
- (void)liveUpdatePreviewTimerSelector:(NSTimer *)theTimer
{
	if ([[SMLDefaults valueForKey:@"LiveUpdatePreview"] boolValue] == YES) {
		[[SMLPreviewController sharedInstance] liveUpdate];
	}
